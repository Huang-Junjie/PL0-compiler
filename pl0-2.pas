...
var
  ...
  mnemonic : array [fct] of packed array [1..5] of char; {中间代码算符的字符串}
...
procedure error (n : integer);
  begin 
    writeln(fileOut, '****', ' ' : cc-1, '↑', n : 2);  err := err + 1
  ...
  procedure  getch ; {取下一字符}
    begin
      if cc = ll then {如果cc指向行末}
        begin
          if eof(fileIn) then {如果已到文件尾}
            begin
              write(fileOut, 'PROGRAM INCOMPLETE'); 
              close(fileIn); 
              close(fileOut);
              exit
            end;
          {读新的一行}
          ll := 0; cc := 0; write(fileOut, cx : 5, ' '); {cx : 5位数}
          while not eoln(fileIn) do {如果不是行末}
            begin
              ll := ll + 1; read(fileIn, ch); write(fileOut, ch);
    ...
    if ch in ['A'..'Z', 'a'..'z'] then 
      begin {标识符或保留字} 
        ...
        if k >= kk  
          ...
          repeat 
            a[kk] := ' '; kk := kk-1 
          until kk = k;                {如果标识符长度不是最大长度, 后面补空白}
        id := a;  i := 1;  j := norw;
        {id中存放当前标识符或保留字的字符串}
...
procedure  gen(x : fct; y, z : integer); 
  begin 
    if cx > cxmax then {如果当前指令序号>代码的最大长度}
      begin 
        write(fileOut, 'PROGRAM TOO LONG'); 
        close(fileIn);
        close(fileOut);
        exit
      end;
...
procedure  test(s1, s2 : symset; n : integer);
  begin
    if not (sym in s1) then {如果当前记号不属于集合S1,则报告错误n}
      begin  
        error(n);  
        s1 := s1 + s2;
        while not (sym in s1) do 
          getsym {跳过一些记号, 直到当前记号属于S1∪S2}
      end
  end {test};
    ...
    begin  {列出本程序体生成的代码}
      for i := cx0 to cx-1 do {cx0: 本过程第一个代码的序号, cx-1: 本过程最后一个代码的序号}
        with code[i] do {打印第i条代码}
          writeln(fileOut, i : 5, mnemonic[f] : 7, l : 3, a : 5)
         {i: 代码序号; 
          mnemonic[f]: 功能码的字符串;
          l: 相对层号(层差);
          a: 相对地址或运算号码}
    ...
    repeat
      ...
      case f of
        ...
        red : begin
            writeln('input: ');
            readln(s[base(l)+a]);
            writeln(fileOut, 'input: ', s[base(l)+a])
          end;
        wrt : begin
            writeln('output: ', s[t]);
            writeln(fileOut, 'output: ', s[t]);
            t := t+1
          end
        end {with, case}
    until p = 0; 
...
begin  {主程序}
  ...
  else write(fileOut, 'ERRORS IN PL/0 PROGRAM');
